...
try {
    if ($role === 'regular') {
        $stmt = $db->prepare('SELECT citizen_id AS id, username, password_hash, first_name, last_name, email, phone_number, home_barangay FROM citizen_accounts WHERE username = :username');
        $stmt->execute([':username' => $username]);
        $user = $stmt->fetch();
        $redirect = 'civilian.html';
